...
- (void)peripheralManagerDidUpdateState:(CBPeripheralManager *)peripheral
{
    switch(peripheral.state)
    {
        case CBPeripheralManagerStatePoweredOff:
            ...
            [self _addToDisplayLog:[NSString stringWithFormat:@"   isAdvertising:%@",@(peripheral.isAdvertising)]];
            break;
    ...
    {
        _isReady = YES;
        [self.delegate viewModelIsReady:self];
    }
}
...
- (void)startAdvertising
{
    ...
    {
        self.startupTimer = [NSTimer scheduledTimerWithTimeInterval:1 target:self selector:@selector(startAdvertising:) userInfo:nil repeats:YES];
    }
}

- (void)startAdvertising:(NSTimer*)timer
{
    [self startAdvertising];
}
